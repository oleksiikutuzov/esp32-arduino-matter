...
/**
 * This program presents example Matter plugin unit (switch) device with OnOff
 * cluster by controlling LED with Matter and toggle button.
 *
 * If your ESP does not have builtin LED, please connect it to LED_PIN
 *
 * You can toggle plugin unit by both:
 *  - Matter (via CHIPTool or other Matter controller)
 *  - toggle button (by default attached to GPIO0, with
 * debouncing)
 */
...
static esp_err_t on_attribute_update(attribute::callback_type_t type,
                                     ...
                                     void *priv_data) {
  if (type == attribute::PRE_UPDATE && endpoint_id == plugin_unit_endpoint_id && cluster_id == CLUSTER_ID && attribute_id == ATTRIBUTE_ID) {
...
void setup() {
  ...
  endpoint_t *endpoint = on_off_plugin_unit::create(node, &plugin_unit_config,
  ...
  plugin_unit_endpoint_id = endpoint::get_id(endpoint);
